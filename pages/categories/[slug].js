...
export async function getStaticProps({ params }) {
    const res = await fetch(`https://emoha.com/blogs/wp-json/wp/v2/categories?slug=${params.slug}`)
    const category = await res.json()
    const postsQuery = await fetch(`https://emoha.com/blogs/wp-json/wp/v2/posts?categories=${category[0].id}?per_page=100`)
